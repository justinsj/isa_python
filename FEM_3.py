...
def plot(node_list,element_list):
    ...
    ax.set_ylim(-1-1.1*max(np.array(node_list)[:, 2:3]), 1+1.1*max(np.array(node_list)[:, 2:3]))      
    ax.set_aspect("equal")
    
def plot_displacement(scale,u_vector):
    global node_list,element_list
    plot(node_list,element_list)
...
#%%
file_des = "C:/Users/jdqsj/Desktop/Workspace/isa-python/"

#%%
#Import ground truth
from Define_Node_Loc_3 import  get_node_and_element_lists
A = 0.02
E = 200000000
I = 0.0002
...
def produce_graphs(fname):
    global node_dis, element_dis,node_list, element_list
    discretize = 1
    temp = []
    node_list,element_list = get_node_and_element_lists(fname)
    ...
    #%%
    plot_displacement(100,u_vector)
    plot_axial_force(1)
    plot_shear_force(1)
    plot_bending_moment(1)
