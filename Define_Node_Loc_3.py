...
def load_items(filepath):
    items = []
    for i in open(filepath).readlines()[1:]:
        item = i.rstrip().split("\t")
        item = list(map(int, item[0].split()))
        items.append(item)
    items = sorted(items, key = lambda x: (x[0]+ 0.25*x[1]))
    return items    
def import_ground_truth(fname):
    #Import ground truth
    GT = "GT_"+str(fname)+".txt"
    return GT

def print_data(node_table, element_table):
    print("\n"+"Node Table: "+"\n")
    print(node_table)
    print("\n"+"Element Table: "+"\n")
    print(element_table)
...
def get_node_and_element_lists(fname):
    #Threshold value
    duplicate_node = 60
    ...
    GT = import_ground_truth(fname)
    filepath = os.path.join(file_des + "test_images", (GT))
...
#%%
    ...
    if plt_show == True:
        ...
        img=mpimg.imread(os.path.join(file_des + "test_images", (fname+".JPG")))
    ...
    if plt_show == True:
        ...
        plt.show()
    print_data(node_table,element_table)
